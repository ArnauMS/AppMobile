...
Future<void> stop(int id) async {
  ...
  if (response.statusCode == 200) {
    ...
    print("statusCode=$response.statusCode");
    throw Exception('Failed to stop activity');
  }
}

Future<void> createChild(int id, String name, String tags, String type) async {
  String uri = "$baseUrl/createchild?$id&$name&$tags&$type";
  final response = await client.get(Uri.parse(uri));
  if (response.statusCode == 200) {
    print("statusCode=$response.statusCode");
  } else {
...
Future<String> searchByTag(String tag) async {
  String uri = "$baseUrl/tag?$tag";
  final response = await client.get(Uri.parse(uri));
  if (response.statusCode == 200) {
    print("statusCode=$response.statusCode");
